...
export const processSubscriptions = async () => {
  ...
  try {
    ...
    for (const sub of dueSubscriptions) {
      ...
      // Ensure we don't duplicate logic. Ideally wrap this in a transaction.
      const session = await mongoose.startSession();
      session.startTransaction();

      try {
        // Find default vendor (e.g. centralized studio kitchen)
        // For demo, we just grab any MnsStudio vendor
        ...
        const order = await Order.create([{
            ...
            paymentMethod: 'online', // Subscriptions usually pre-paid/auto-debited
            sourceMode: 'studio',
